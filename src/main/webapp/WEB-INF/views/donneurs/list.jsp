    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-gray-600 text-sm">Manage all donors in the system</p>
            </div>
            <a href="${pageContext.request.contextPath}/donneur?action=add" class="inline-flex items-center px-4 py-2 rounded-md bg-primary-600 text-white hover:bg-primary-700">Add new donor</a>
        </div>
    
    <c:choose>
        <c:when test="${empty donneurs}">
            <div class="bg-white rounded-lg border shadow-sm p-8 text-center text-gray-600">
                No donors yet. <a class="text-primary-700 hover:underline" href="${pageContext.request.contextPath}/donneur?action=add">Add the first donor</a>
            </div>
        </c:when>
        <c:otherwise>
                    ...
                    <table class="min-w-full text-sm">
                        ...
                        <tbody>
                            <c:forEach var="donneur" items="${donneurs}">
                                <tr class="border-t">
                                    <td class="px-4 py-3">${donneur.id}</td>
                                    <td class="px-4 py-3">${donneur.nom} ${donneur.prenom}</td>
                                    <td class="px-4 py-3">${donneur.telephone}</td>
                                    <td class="px-4 py-3">${donneur.cin}</td>
                                    <td class="px-4 py-3">${donneur.dateNaissance}</td>
                                    <td class="px-4 py-3">${donneur.poids} kg</td>
                                    <td class="px-4 py-3">${donneur.sexe}</td>
                                    <td class="px-4 py-3">${donneur.groupeSanguin}</td>
                                    <td class="px-4 py-3">${donneur.statutDisponibilite}</td>
                                    <td class="px-4 py-3">
                                        <a href="${pageContext.request.contextPath}/donneur?action=edit&id=${donneur.id}" class="text-primary-700 hover:underline">Edit</a>
                                        <span class="mx-2 text-gray-300">|</span>
                                        <a href="${pageContext.request.contextPath}/donneur?action=delete&id=${donneur.id}" class="text-red-600 hover:underline" onclick="return confirm('Are you sure?')">Delete</a>
                                    </td>
                                </tr>
                            </c:forEach>
                        </tbody>
